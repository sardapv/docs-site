...
From this aim, we meed to create 2 parts first.
...
After creating new project, We are ready to edit and start making changes.
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      // and it's primary color is blue.
      theme: ThemeData(
...
- In Simple terms, `Scaffold` is a drawing sheet on drawing pad called `MaterialApp` which will give 
material support to the whole app.

### StatefulWidget ###

The widget whose state can be altered/changed/updated once it is built (mutable state).
We will be working on them in further process.

- Coming back to code, Let's edit `Calculator` Widget.
...
class _CalculatorState extends State<Calculator> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      // in safe area from overlaying the OS's 
      ...
      body: SafeArea(
        child: Center(
          child: Column(
            children: <Widget>[
              Container(
                // Height of the container is 30% of it's screen height.
                ...
                // We need a height of the screen, so we used seice of height(size.height).
                // To make this container responsive we are 
                // multiplying the hight with 0.3 times of the screen height.
                // This code is only for testing purpose. The values may change in future.
              ...
              Container(
                // Height of the container is 70% of it's screen height.
...
:::warning
If the dart files you created aren't in `lib` folder, then the files cannot be read. 
...
class CustomButtons extends StatelessWidget {
  ...
  // Then it will return a red screen with the error stated below.
   assert(
          // Condition. If this doesn't satisy this error will be returned.
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      // we pass Text as child to get numbers.
      ...
      child: ElevatedButton(
        ...
        // That will be shown in next code block. 
        style: ElevatedButton.styleFrom(
          // By using Ternary operator we will write a condition.
          // if(buttonColor == null)
          // {
          //  if you haven't passed any color then this 
          ...
          // will make a cut with radii length arc.
        ...
        // It can't be null cause we wrote assert at constructor.
...
- Now we are ready to use This widget. In `Caluculator` class we are adding these buttons
with in `Row`s and `Column`s.
...
above the build function with `String` parameter passign in it.
...
class _CalculatorState extends State<Calculator> {
  ...
  void numPressed(String text){
    // OnPressed the button, we will setState
...
- Call `numPressed` in onPressed property of all `CustomButtons` widget. Check what it prints.
...
class _CalculatorState extends State<Calculator> {
  ...
  void evaluate(String text) {
    ...
    // evaluated result and converting that as string.
    setState(() {
      _operation = _result;
...
- As we generally don't use calculator in landscape mode, we will disable the orientation.
